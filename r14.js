...
(function(){
  ...
  function prepInstruction(n){
    ...
    if(x.includes('smuti')||x.includes('smoothie')||x.includes('matcha skleda')) return 'Operi in odmeri sadje ter pripravi dodatke. Zamrznjene sestavine lahko razdeliš v posodico za blender.';
    if(x.includes('palačink')||x.includes('vafelj')||x.includes('šmorn')||x.includes('french toast')) return 'Odmeri suhe sestavine in pripravi prelive/dodatke. Testo oziroma jajčno zmes dokončaj tik pred peko.';
    ...
    if(x.includes('losos')||x.includes('ribji')||x.includes('riba')) return 'Če je riba zamrznjena, jo prestavi v hladilnik. Pripravi zelenjavo, prilogo in omako; ribo speci naslednji dan.';
    if(x.includes('tuna')||x.includes('tunine')||x.includes('tunin')) return 'Pripravi zelenjavo in dodatke. Tuno odcedi in zmešaj čim bližje obroku, posebej pri solati ali namazu.';
    ...
    if(x.includes('curry')||x.includes('daal')) return 'Nareži zelenjavo, odmeri začimbe in pripravi stročnice/beljakovine. Omako lahko delno ali v celoti skuhaš že zvečer.';
    ...
    if(x.includes('njoki')||x.includes('orzo')||x.includes('testenine')) return 'Pripravi omako, nareži zelenjavo in odmeri testenine/njoke. Skuhaj jih naslednji dan, razen za hladno solato.';
  ...
  function renderSmartPrep(){
    if(typeof weeks==='undefined'||typeof wi==='undefined') return;
    const w=weeks[wi], host=document.getElementById('pg');
    if(!w||!host) return;
  ...
  function refreshNotes(){
    ...
    if(prepTip) prepTip.textContent='Kaj pripraviš večer prej za naslednji dan. Kljukice se shranijo v brskalniku; stvari, ki so boljše sveže, pripraviš le delno.';
    const shopTip=document.querySelector('#shop .tip');
    if(shopTip) shopTip.textContent='Nakupovalni seznam se menja glede na izbrani teden in je pripravljen za 2 odrasla + 2 otroka. Količine so za zdaj okvirne.';
  }
